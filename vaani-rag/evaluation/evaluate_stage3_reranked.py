...
RESULTS_FILE = PROJECT_ROOT / "evaluation" / "results" / "stage3_reranked_results.jsonl"

def main():
    print("=" * 80)
    print("VAANIRAG STAGE 3 RERANKED EVALUATION")
    print("=" * 80)

    # 1. Initialize RAG pipeline with Reranker
    print("\nInitializing Pipeline (loading OpenVINO BGE-M3 and BGE-Reranker-v2-m3 models)...")
    try:
        # Load local embedder
        embedder = BGEM3Embedder()
        reranker = BGEM3Reranker(device="GPU")
        pipeline = RAGBaselinePipeline(embedder=embedder, reranker=reranker)
    ...
    # 4. Evaluate queries
    print("\nRunning RAG generation with reranking...")
    evaluated_count = 0
    ...
    with open(RESULTS_FILE, "w", encoding="utf-8") as out:
        for idx, item in enumerate(queries, 1):
            ...
            t0 = time.time()
            response = pipeline.generate_answer(query=q_text, language=q_lang, top_k=5, rerank_candidates=20)
            elapsed = time.time() - t0
            
            print(f"  Status: {'SUCCESS' if response.success else 'FAILED'}")
            print(f"  Answer: {response.answer}")
            print(f"  Reranked Chunks (Top-5 selected from 20): {len(response.reranked_candidates)}")
            print(f"  Latency Profiles:")
            print(f"    Retrieval: {response.telemetry.retrieval_ms:.2f} ms")
            print(f"    Reranking: {response.telemetry.reranking_ms:.2f} ms")
                ...
                "reranked_candidates": [
                    {
                        ...
                        "text": c.text,
                        "language": c.language,
                        "original_score": c.retrieval_score,
                        "rerank_score": c.rerank_score
                    }
                    for c in response.reranked_candidates
                ],
                "telemetry": {
                    "retrieval_ms": response.telemetry.retrieval_ms,
                    "reranking_ms": response.telemetry.reranking_ms,
            ...
            if not response.success and "429" in (response.error_message or ""):
                print("  Rate Limit Hit (429)! Backing off for 40 seconds...")
                time.sleep(40.0)
            else:
                time.sleep(4.5)

    print("\n" + "=" * 80)
    print(f"STAGE 3 RERANKED RAG EVALUATION COMPLETE: {evaluated_count} queries processed.")
